...
class RideSelection {
  final GeocodedPlace source;
  final GeocodedPlace destination;

  const RideSelection({required this.source, required this.destination});
}
...
class SearchScreen extends StatefulWidget {
  ...
  const SearchScreen({
    super.key,
    this.currentAddress = 'Your location',
    this.currentLat,
    this.currentLng,
  });
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  GeocodedPlace get _currentSource => GeocodedPlace(
        formatted: widget.currentAddress,
        lat: widget.currentLat ?? _defaultLat,
        lng: widget.currentLng ?? _defaultLng,
      );
  ...
  void _selectGeocoded(GeocodedPlace p) {
    Navigator.pop(
      context,
      RideSelection(source: _currentSource, destination: p),
    );
  }

  // ── Set on map (two-step: source → destination) ─────────────────────────────

  Future<void> _setOnMap() async {
    final source = await Navigator.push<GeocodedPlace>(
      context,
      MaterialPageRoute(
    ...
    final dest = await Navigator.push<GeocodedPlace>(
      context,
      MaterialPageRoute(
        builder: (_) => MapPickerScreen(
          title: 'Set Drop',
          initialLat: source.lat,
          initialLng: source.lng,
        ),
      ),
    );
    ...
    Navigator.pop(
      context,
      RideSelection(source: source, destination: dest),
    );
  }

  // ── List items ─────────────────────────────────────────────────────────────

  static const _mapOptionKey = '_MAP_OPTION_';

  List<Object> get _listItems {
    final List<Object> items = [_mapOptionKey];
    if (_query.isEmpty) {
      return [
        ...items,
        'RECENT PLACES',
    ...
    if (_geocodeResults != null && _geocodeResults!.isNotEmpty) {
      return [...items, 'SEARCH RESULTS', ..._geocodeResults!];
    }
          ...
          Expanded(
            child: items.length <= 1
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final item = items[i];
                      if (item == _mapOptionKey) {
                        return _MapOptionTile(onTap: _setOnMap);
                      }
                      if (item is String) {
...
class _MapOptionTile extends StatelessWidget {
  ...
  const _MapOptionTile({required this.onTap});
            ...
            const Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 2),
                  Text(
                    'Drag the map to pin exact locations',
                    style: TextStyle(
